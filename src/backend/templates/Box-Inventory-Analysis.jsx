...
const StorageBoxSimpleTableReport = ({ data }) => {
  ...
  const totalItems = processedData.reduce((sum, box) => sum + box.numberOfItems, 0);
  
  // Calculate unique tags
  const allTagsSet = new Set();
  data.forEach(box => {
    if (box.tags && Array.isArray(box.tags)) {
      box.tags.forEach(tag => allTagsSet.add(tag));
    }
  });
  const totalUniqueTags = allTagsSet.size;
        ...
        function initTable() {
          ...
          function sortTable(key, direction) {
            ...
            rows.sort((a, b) => {
              ...
              if (key === 'rownumber' || key === 'numberofitems') {
            ...
            React.createElement('tbody', { key: 'tbody' },
              processedData.map((box, idx) => 
                React.createElement('tr', {
                  ...
                  'data-numberofitems': box.numberOfItems,
                  'data-tagnames': box.tagNames,
                  ...
                  React.createElement('td', { 
                    ...
                    style: { 
                      ...
                      fontWeight: 'bold',
                      fontSize: '18px',
                      color: box.numberOfItems > 0 ? '#52B788' : '#999'
                    } 
                  }, box.numberOfItems),
                  React.createElement('td', { 
                    key: 'tags', 
                  ...
                  }, box.tagNames || React.createElement('em', { style: { color: '#999' } }, 'No tags'))
                ])
              )
            )
          ])
...
module.exports = StorageBoxSimpleTableReport;